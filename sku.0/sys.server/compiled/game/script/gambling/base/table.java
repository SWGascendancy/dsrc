...
public class table extends script.gambling.base.default_interface
{
    ...
    public int handlePlayerAdded(obj_id self, dictionary params)
    {
    ...
    public int handleBetFailed(obj_id self, dictionary params)
    {
        if (params == null || params.isEmpty())
        {
            return SCRIPT_CONTINUE;
    ...
    public int handleRequestUpdatedUI(obj_id self, dictionary params)
    {
        if (params == null || params.isEmpty())
        {
            return SCRIPT_CONTINUE;
    ...
    public int handleBetUi(obj_id self, dictionary params)
    {
        if (params == null || params.isEmpty())
        {
            return SCRIPT_CONTINUE;
    ...
    public int handleBetTimer(obj_id self, dictionary params)
    {
        if (!utils.hasScriptVar(self, gambling.VAR_TABLE_BET_ACCEPT))
        {
            return SCRIPT_CONTINUE;
    ...
    protected void startTableBetting(obj_id self)
    {
        obj_id[] players = getObjIdArrayObjVar(self, gambling.VAR_TABLE_PLAYERS);
        if (players == null || players.length == 0)
        {
    ...
    protected void stopTableGame(obj_id self)
    {
        utils.removeScriptVar(self, gambling.VAR_TABLE_BET_ACCEPT);
        removeObjVar(self, gambling.VAR_GAME_BASE);
    }
    protected boolean updateBetSUI(obj_id table, obj_id player)
    {
    ...
    public boolean closeBetSUI(obj_id table, obj_id player)
    {
        if (!isIdValid(table) || !isIdValid(player))
        {
            return false;
    ...
    protected boolean sendTableMessage(obj_id table, prose_package pp, obj_id ommit_player)
    {
        if (!isIdValid(table))
        {
            return false;
    ...
    protected boolean sendTableMessage(obj_id table, prose_package pp)
    {
        return sendTableMessage(table, pp, null);
    }
    public boolean sendTableMessage(obj_id table, string_id message, obj_id ommit_player)
    {
        return sendTableMessage(table, prose.getPackage(message, table), ommit_player);
    }
    protected boolean sendTableMessage(obj_id table, string_id message)
    {
