...
public class foraging_enemy extends script.base_script
{
    ...
    public int expression(obj_id self, dictionary params)
    {
        obj_id player = getObjIdObjVar(self, "player");
        if (!isValidId(player) || !exists(player))
        {
    ...
    public int runAwayThenCleanUpGuard(obj_id self, dictionary params)
    {
        pathTo(self, groundquests.getRandom2DLocationAroundPlayer(self, 20, 50));
        messageTo(self, "cleanUpGuard", null, 12, true);
        return SCRIPT_CONTINUE;
    }
    public int attackForager(obj_id self, dictionary params)
    {
        obj_id player = getObjIdObjVar(self, "player");
        if (!isValidId(player) || !exists(player))
        {
    ...
    public boolean isThief(obj_id mob) {
        if (!isValidId(mob)) {
            return false;
        }
    ...
    public boolean removeFirstSlotFound(obj_id player)
    {
        if (!isValidId(player))
        {
            return false;
    ...
    public int barkDefeat(obj_id self, dictionary params)
    {
        if (params == null)
        {
            return SCRIPT_CONTINUE;
    ...
    public int removeASlot(obj_id self, dictionary params)
    {
        if (params == null)
        {
            return SCRIPT_CONTINUE;
