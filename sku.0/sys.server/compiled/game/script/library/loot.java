...
public class loot extends script.base_script
{
    ...
    private static final float rlsDropRate = utils.getFloatConfigSetting("GameServer", "rlsDropRate");
    private static final float rareLootChance = 7 * rlsDropRate;
    private static final float exceptionalLootChance = 3 * rlsDropRate;
    private static final float legendaryLootChance = 1 * rlsDropRate;
    ...
    public static int getCashForLevel(String mobType, int level)
    {
    ...
    public static boolean addResourceLoot(obj_id target)
    {
    ...
    public static int getCalculatedAttribute(int minVal, int maxVal, int creatureLevel, int minDropLevel, int maxDropLevel)
    {
        return Math.round(getCalculatedAttribute((float)minVal, (float)maxVal, creatureLevel, minDropLevel, maxDropLevel));
    }
    public static float getCalculatedAttribute(float minVal, float maxVal, int creatureLevel, int minDropLevel, int maxDropLevel)
    {
    ...
    public static int getCalculatedAttribute(int minVal, int maxVal, int creatureLevel)
    {
        return getCalculatedAttribute(minVal, maxVal, creatureLevel, MIN_CREATURE_LEVEL, MAX_CREATURE_LEVEL);
    }
    public static float getCalculatedAttribute(float minVal, float maxVal, int creatureLevel)
    {
        return getCalculatedAttribute(minVal, maxVal, creatureLevel, MIN_CREATURE_LEVEL, MAX_CREATURE_LEVEL);
    }
    public static int getRowIndexForComponent(String componentTemplate)
    {
    ...
    public static dictionary getComponentData(obj_id component)
    {
        return getComponentData(getTemplateName(component));
    }
    public static dictionary getComponentData(String componentTemplate)
    {
    ...
    public static boolean redeemFactionCoupon(obj_id redeemer, obj_id player, obj_id coupon)
    {
    ...
    public static int getFactionCouponValue(obj_id redeemer, obj_id coupon)
    {
    ...
    public static boolean isFactionalDataItem(obj_id item)
    {
    ...
    public static obj_id[] getFactionalDataItems(obj_id target, String faction)
    {
    ...
    public static obj_id[] getFactionalDataItems(obj_id target)
    {
        return getFactionalDataItems(target, "");
    }
    public static boolean randomizeWeapon(obj_id weapon, int creatureLevel)
    {
        if (!isIdValid(weapon))
        {
            return false;
    ...
    public static boolean randomizeArmor(obj_id item, int level)
    {
    ...
    public static boolean randomizeMedicine(obj_id item, int level)
    {
        if (!isIdValid(item))
        {
            return false;
    ...
    public static float[] validateComponentVals(float[] dat)
    {
    ...
    public static boolean randomizeComponent(obj_id item, int level, obj_id container)
    {
        if (!isIdValid(item))
        {
            return false;
    ...
    public static String getCreatureType(String name)
    {
        return dataTableGetString("datatables/mob/creatures.iff", name, "lootList");
    }
    public static int getAdjustedCreatureLevel(obj_id target, String name)
    {
    ...
    public static boolean generateTheftLootRare(obj_id container, obj_id mark, int maxItems, obj_id thief)
    {
    ...
    public static boolean generateTheftLoot(obj_id container, obj_id mark, float chanceMod, int maxItems)
    {
        String name = getCreatureName(mark);
        String type = getCreatureType(name);
        if (type == null || type.equals("none"))
    ...
    public static int getNpcMoney(obj_id target)
    {
        String mobType = ai_lib.getCreatureName(target);
        if (mobType == null)
        {
    ...
    public static boolean addCashAsLoot(obj_id target, int cash)
    {
        if (cash < 1)
        {
            return false;
    ...
    public static boolean doGroupLooting(obj_id corpseId, obj_id transferer, obj_id item)
    {
    ...
    public static boolean doGroupLootAllCheck(obj_id player, obj_id corpseId)
    {
    ...
    public static boolean pickRandomLooter(obj_id player, obj_id team, obj_id corpse)
    {
    ...
    public static void sendGroupLootSystemMessage(obj_id item, obj_id winner, String stf, String message)
    {
        sendGroupLootSystemMessage(item, winner, stf, message, false);
        return;
    }
    public static void sendGroupLootSystemMessage(obj_id item, obj_id winner, String stf, String message, boolean skipWinner)
    {
    ...
    public static void setAutoLootComplete(obj_id winner, obj_id corpse, obj_id item)
    {
        setObjVar(corpse, "autoLootComplete", 1);
        setObjVar(item, "pickupable", winner);
        return;
    }
    public static obj_id chooseRandomLootPlayerFromGroup(obj_id corpse, obj_id player)
    {
    ...
    public static boolean isMasterLooter(obj_id toTest, boolean sendMessage)
    {
    ...
    public static void lootAiCorpse(obj_id self, obj_id target)
    {
        if (!isIdValid(target))
        {
            return;
    ...
    public static obj_id findItemToStack(obj_id item)
    {
    ...
    public static void stackItem(obj_id objSourceItem, obj_id objDestinationItem)
    {
    ...
    public static boolean setupLootItems(obj_id objCreature)
    {
    ...
    public static boolean setupLootItems(obj_id objCreature, int intItems)
    {
        obj_id objContainer = utils.getInventoryContainer(objCreature);
        if (!hasObjVar(objCreature, "loot.lootTable"))
        {
    ...
    public static boolean makeLootInContainer(obj_id objContainer, String strTable, int intItems, int intLevel)
    {
    ...
    public static obj_id makeRlsItem(obj_id objContainer, String strTable)
    {
        String strRootItems = "datatables/loot/loot_items/";
        String[] parse = split(strTable, ':');
        strTable = parse[0];
        ...
        if (dataTableOpen(strItemTable)) {
            ...
            {
                String strLootToMake = strItems[rand(0, strItems.length - 1)];
                obj_id item = createLootItem(objContainer, strLootToMake, 1);
                if (!hasScript(item, "item.static_item_base"))
                    attachScript(item, "item.static_item_base");
                int rarity = 3;
                if (parse[0].contains("space_rare"))
                    rarity = 4;
                else if (parse[0].contains("space_exceptional"))
                    rarity = 5;
                else if (parse[0].contains("space_legendary"))
                    rarity = 6;
                else if (parse[0].contains("rare"))
                    rarity = 1;
                else if (parse[0].contains("exceptional"))
                    rarity = 2;
                setObjVar(item, RareLootSystem.RARITY_OBJVAR, rarity);
                return item;
            }
        }
        return null;
    }
    public static obj_id createLootItem(obj_id objContainer, String strLootToMake, int intLevel)
    {
    ...
    public static boolean addMilkOrEgg(obj_id objCreature)
    {
        if (rand(1, 100) > 20)
        {
            return false;
    ...
    public static void notifyThiefOfItemStolen(obj_id objContainer, obj_id loot)
    {
        if (utils.hasScriptVar(objContainer, "theft_in_progress"))
        {
    ...
    public static boolean addBeastEnzymes(obj_id creature)
    {
    ...
    public static boolean addChronicleLoot(obj_id target)
    {
    ...
    public static String getChronicleRelicLootOfCategory(String lootCatergoryChoice)
    {
        String relic = "";
    ...
    public static obj_id chroniclesCraftingLootDrop(obj_id player)
    {
    ...
    public static obj_id chroniclesPvpLootDrop(obj_id player)
    {
        if (hasToggledChroniclesLootOff(player))
        {
            return obj_id.NULL_ID;
    ...
    public static obj_id chroniclesNonCorpseLootDrop(obj_id player, String relicCategory, int relicChance, String activityType)
    {
        String configChance_string = getConfigSetting("GameServer", "chroniclesLootChanceOverride");
        if (configChance_string != null && configChance_string.length() > 0)
        {
    ...
    public static boolean hasToggledChroniclesLootOff(obj_id player)
    {
        return hasObjVar(player, CHRONICLES_LOOT_TOGGLE_OBJVAR);
    }
    public static void disableChroniclesLoot(obj_id player)
    {
        setObjVar(player, CHRONICLES_LOOT_TOGGLE_OBJVAR, true);
        return;
    }
    public static void enableChroniclesLoot(obj_id player)
    {
        removeObjVar(player, CHRONICLES_LOOT_TOGGLE_OBJVAR);
        return;
    }
    public static boolean addCollectionLoot(obj_id target)
    {
        return addCollectionLoot(target, false, null);
    }
    public static boolean addCollectionLoot(obj_id target, boolean theftBool, obj_id thief)
    {
        String creatureName = ai_lib.getCreatureName(target);
    ...
    public static String getFactionalCollectionItem(String faction)
    {
    ...
    public static String getAppropriatePvpRankCollectible(String faction, int rank)
    {
        if (faction == null || faction.equals(""))
        {
            return null;
    ...
    public static boolean rollRandomFactionalCollectible(obj_id victim, obj_id winner, int victimRank)
    {
    ...
    public static boolean grantFactionalCollectible(obj_id winner, String staticItem)
    {
        if (isIdNull(winner))
        {
            return false;
    ...
    public static obj_id addMeatlumpLumpsAsLoot(obj_id target, obj_id targetInventory, int cash)
    {
    ...
    public static boolean giveMeatlumpPuzzleLoot(obj_id player, boolean puzzleThreshold, boolean puzzleBuff)
    {
    ...
    public static int getRandomRareForagedLoot(obj_id self, int enzymeChance, int wormChance, int treasureMapChance, int componentChance)
    {
    ...
    public static boolean getRareForagedTreasureMap(obj_id player, obj_id pInv)
    {
    ...
    public static boolean giveForagedCollectionObject(obj_id self, obj_id pInv, String scene)
    {
        if (!isIdValid(self))
        {
            return false;
    ...
    public static boolean saveForageLocationOnPlayer(obj_id self, Vector newListOfLocs, location curLoc)
    {
    ...
    public static boolean logForageData(obj_id player)
    {
        if (!isValidId(player) || !exists(player))
        {
            return false;
    ...
    public static boolean forage_blog(String msg)
    {
    ...
    {
        obj_id player = getObjIdObjVar(target, xp.VAR_LANDED_DEATHBLOW);

        if (isAwayFromKeyBoard(player) || !isPlayerActive(player)) {
            return false;
        }
        ...
        if (creatureName == null || (playerLevel - mobLevel) > 6) {
            return false;
        }
        ...
        if (mobLevel >= playerLevel) {
            ...
            switch (dataTableGetInt("datatables/mob/creatures.iff", creatureName, "difficultyClass")) {
                case 1:
                    ...
                    bonus += 2 * (mobLevel - playerLevel + 1);
                    break;
                case 2:
                    if (creatureName.startsWith("heroic_")) {
                        bonus += 50;
            ...
            bonus -= 2 * (playerLevel - mobLevel);
        }

        int rareLootRoll = rand(1, 1000 - bonus);

        if (rareLootRoll > rareLootChance)
            return false;

        int lootType = 0;

        if (rareLootRoll <= legendaryLootChance) {
            lootType = 3;
        } else if (rareLootRoll <= exceptionalLootChance) {
            lootType = 2;
        } else {
            lootType = 1;
        }

        obj_id chest = static_item.createNewItemFunction(CHEST_BASE + lootType, utils.getInventoryContainer(target));
        setObjVar(chest, RareLootSystem.RARITY_OBJVAR, lootType);
        playClientEffectLoc(target, RLS_EFFECT, getLocation(target), 1f);
        playClientEffectLoc(target, RLS_SOUND, getLocation(target), 1f);
        return true;
    }
}
