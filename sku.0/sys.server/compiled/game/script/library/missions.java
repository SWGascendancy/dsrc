...
public class missions extends script.base_script
{
    ...
    public static final int DAILY_MISSION_XP_REWARD = 30;
    public static final int DAILY_MISSION_CASH_REWARD = 45;
    ...
    public static void sendBountyFail(obj_id hunter, obj_id target)
    {
    ...
    public static void sendBountySuccess(obj_id hunter, obj_id target)
    {
        dictionary params = new dictionary();
        params.put("target", target);
        setObjVar(hunter, "intState", STATE_MISSION_COMPLETE);
    ...
    public static void sendBountyIncomplete(obj_id hunter, obj_id target)
    {
        dictionary params = new dictionary();
        params.put("target", target);
        setObjVar(hunter, "intState", STATE_MISSION_COMPLETE);
    ...
    public static void increaseBountyJediKillTracking(obj_id objPlayer, int stat)
    {
    ...
    public static int getPlayerDailyCount(obj_id player)
    {
        if (!isIdValid(player))
        {
            return 0;
    ...
    public static void incrementDaily(obj_id player)
    {
    ...
    public static boolean canEarnDailyMissionXp(obj_id player)
    {
        if (!isIdValid(player))
        {
            return false;
    ...
    public static void initializeDailyOnLogin(obj_id player)
    {
        if (hasObjVar(player, DAILY_MISSION_CLOCK_OBJVAR))
        {
            int currentTime = getCalendarTime();
    ...
    public static void clearDailyObjVars(obj_id player)
    {
        setObjVar(player, DAILY_MISSION_OBJVAR, 0);
        removeObjVar(player, DAILY_MISSION_CLOCK_OBJVAR);
    }
    public static boolean isDestroyMission(obj_id objMissionData)
    {
