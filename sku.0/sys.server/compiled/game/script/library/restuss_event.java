...
public class restuss_event extends script.base_script
{
    ...
    private static final Vector accountsInRestuss = new Vector();
    ...
    public static void playMusicInArea(obj_id controller, String music)
    {
        playMusicInArea(controller, music, 50.0f);
    }
    public static void playMusicInArea(obj_id controller, String music, float range)
    {
    }
    public static void sendAreaSystemMessage(obj_id controller, string_id message)
    {
    }
    public static boolean getIsStatic(obj_id subject)
    {
    ...
    public static void setIsStatic(obj_id subject, boolean state)
    {
        utils.setScriptVar(subject, IS_STATIC, state);
    }
    public static int getPhase(obj_id object)
    {
    ...
    public static void incrimentPhase(obj_id object)
    {
        if (hasObjVar(object, "base_builder.current_phase"))
        {
    ...
    public static void decrimentPhase(obj_id object)
    {
        if (hasObjVar(object, "base_builder.current_phase"))
        {
    ...
    public static int getRequiredQuestNumber(obj_id subject, String questName)
    {
        return dataTableGetInt(QUEST_TABLE, questName, "completion_number");
    }
    public static int getCompletedQuestCount(obj_id subject, String questName)
    {
        int cQuest = 0;
    ...
    public static float getCompletedQuestRatio(obj_id subject, String questName)
    {
        return (float)getCompletedQuestCount(subject, questName) / getRequiredQuestNumber(subject, questName);
    }
    public static void setCompletedQuestCount(obj_id subject, String questName, int number)
    {
    ...
    public static void incrimentCompletedQuestCount(obj_id subject, String questName)
    {
        int current = getCompletedQuestCount(subject, questName);
        current += 1;
        setCompletedQuestCount(trial.getParent(subject), questName, current);
    }
    public static void decrimentCompletedQuestCount(obj_id subject, String questName)
    {
        int current = getCompletedQuestCount(subject, questName);
        current = current - 1;
        if (current < 0)
    ...
    public static boolean isRequiredCountMet(obj_id subject, String questName)
    {
        return (getCompletedQuestRatio(subject, questName) == 1);
    }
    public static int getWinningFaction(obj_id self)
    {
    ...
    public static boolean grantEventBadge(obj_id player)
    {
    ...
    public static boolean isRestussInStageThree(obj_id npc)
    {
    ...
    public static int getRestussPhase(obj_id npc)
    {
        obj_id top = trial.getTop(npc);
        obj_id[] controller_id = getAllObjectsWithObjVar(getLocation(top), 1500.0f, "element");
        obj_id restuss_controller = null;
