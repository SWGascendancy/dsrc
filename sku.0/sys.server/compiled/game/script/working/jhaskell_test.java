...
public class jhaskell_test extends script.base_script
{
    ...
    public int OnSpeaking(obj_id self, String text)
    {
        if (isGod(self))
        {
            ...
            if (tok.hasMoreTokens())
            {
                ...
                else if (priCommand.equalsIgnoreCase("remove_collection_by_name") && !secCommand.equals("") && !thirdCommand.equals(""))
                {
					obj_id thirdCmd = utils.stringToObjId(thirdCommand);
                    if (!isValidId(thirdCmd) || !exists(thirdCmd))
                    {
                        sendSystemMessageTestingOnly(self, "Couldn't find object with OID of: " + thirdCommand);
                        return SCRIPT_CONTINUE;
					}
                    collection.removeCollectionForRealsies(thirdCmd, secCommand);
                }
            }
        }
        return SCRIPT_CONTINUE;
    ...
    public boolean setCitySkillTrainers(obj_id self, int city_id)
    {
    ...
    public boolean destroyCitySkillTrainers(obj_id self, int city_id)
    {
        obj_id[] structures = cityGetStructureIds(city_id);
        for (int i = 0; i < structures.length; i++)
        {
    ...
    public obj_id findTarget(obj_id self)
    {
    ...
    public int getAdjustedLotCount(String template)
    {
    ...
    public boolean isDeluxSarlacc(obj_id sarlacc)
    {
    ...
    public boolean isMassiffBowl(obj_id massifBowl)
    {
    ...
    public obj_id getCurrentAdvancedCamp(obj_id player)
    {
