...
public class lair_destruct extends script.base_script
{
    ...
    public int destroyDisabledLair(obj_id self, dictionary params)
    {
        destroyObject(self);
        return SCRIPT_CONTINUE;
    }
    ...
    public void spawnWaveOfDefenders(obj_id lair, int curHP)
    {
    ...
    public void spawnBossMonster(obj_id lair)
    {
        if (!isIdValid(lair))
        {
            return;
    ...
    public void callForHealing(obj_id lair, int curHP, int maxHP)
    {
        if (curHP == maxHP)
        {
            return;
    ...
    public void sendAlarm(obj_id lair, obj_id attacker)
    {
    ...
    public void setLairFaction(obj_id lair)
    {
        obj_id poiBaseObject = getObjIdObjVar(lair, "poi.baseObject");
        if (isIdValid(poiBaseObject))
        {
