...
public class interplanetary_survey extends script.base_script
{
    ...
    public int handleSurveySelection(obj_id self, dictionary params)
    {
        if (params == null || params.isEmpty())
        {
            cleanScriptVars();
    ...
    public String[] getAvailableSurveyTools(obj_id player)
    {
    ...
    public obj_id[] getAvailablePlanetResources(String planet, String resourceClass)
    {
    ...
    public int getSurveyTime()
    {
        ...
        utils.setScriptVar(getSelf(), "surveyToolGameTime", intTime + time);
        sendResourceListByMail(self);
        return time;
    }
    ...
    public int sendResourceListByMail(obj_id self)
    {
        int intTime = getGameTime();
        int time = getIntObjVar(self, "surveyToolGameTime");
        dictionary params = new dictionary();
        while (hasObjVar(self, "surveyToolGameTime")) 
        {
            if (intTime < time)
            {
                ...
                //utils.sendMail("subject", "body", self, "Survey Droid"); - Iosnowore: Skyyyr you cannot use strings here, you must create string_ids (see the line below).
                // Format: utils.sendMail(string_id, string_id, obj_id, String)
                removeObjVar(self, "surveyToolGameTime");
            }
        }
        return SCRIPT_CONTINUE;
    }
