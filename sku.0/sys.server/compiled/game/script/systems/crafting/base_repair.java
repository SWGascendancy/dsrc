...
public class base_repair extends script.base_script
{
    ...
    public int handleRepair(obj_id self, dictionary params)
    {
    ...
    public float getRepairComplexity(obj_id player, obj_id target)
    {
    ...
    public void disableRepairTarget(obj_id target)
    {
        if (isIdValid(target))
        {
            int got = getGameObjectType(target);
    ...
    public int validateMaxHp(int maxHp, obj_id object)
    {
        if (maxHp <= 1)
        {
            maxHp = 1;
