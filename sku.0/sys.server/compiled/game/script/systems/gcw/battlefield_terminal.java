...
public class battlefield_terminal extends script.base_script
{
    ...
    public int registerTerminal(obj_id self, dictionary params)
    {
        registerTerminalWithController(self);
        return SCRIPT_CONTINUE;
    }
    public obj_id getLocalBattlefieldController(obj_id self)
    {
    ...
    public void startControlAttempt(obj_id self, obj_id player, String terminalFaction)
    {
        int gameTime = getGameTime();
    ...
    public String convertIntFactionToString(int faction)
    {
        if (faction == 1)
        {
    ...
    public void registerTerminalWithController(obj_id self)
    {
        obj_id controller = getLocalBattlefieldController(self);
        if (!isIdValid(controller) || !exists(controller))
        {
    ...
    public void cleanupWaypoint(obj_id controller, obj_id terminal)
    {
        if (!isIdValid(controller) || !exists(controller))
        {
            return;
    ...
    public int handleControlAttemptResults(obj_id self, dictionary params)
    {
    ...
    public void updateMapLabel(obj_id self)
    {
    ...
    public void terminalReset(obj_id self)
    {
        String iconTemplate = "";
        obj_id controller = getLocalBattlefieldController(self);
        if (!isIdValid(controller) || !exists(controller))
    ...
    public int receiveBattlefieldReset(obj_id self, dictionary params)
    {
        terminalReset(self);
        return SCRIPT_CONTINUE;
    }
    public void playEffectAtLoc(String effect, location loc, obj_id[] playerList)
    {
        if (loc == null)
        {
            return;
    ...
    public void playEffectAtLoc(String effect, location loc, Vector playerList)
    {
        if (loc == null)
        {
            return;
    ...
    public void blowUpBuilding(obj_id self)
    {
    ...
    public int effectDelay(obj_id self, dictionary params)
    {
        if (params == null)
        {
            return SCRIPT_CONTINUE;
    ...
    public int sparkTerminal(obj_id self, dictionary params)
    {
        location loc = getLocation(self);
        obj_id controller = getLocalBattlefieldController(self);
        if (!isIdValid(controller) || !exists(controller))
    ...
    public int buildingSmoke(obj_id self, dictionary params)
    {
        obj_id topMost = getTopMostContainer(self);
        if (!isIdValid(topMost) || !exists(topMost))
        {
