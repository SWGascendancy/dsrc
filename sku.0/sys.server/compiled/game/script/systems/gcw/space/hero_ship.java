...
public class hero_ship extends support_ship {
    ...
    public int OnAboutToBeDestroyed(obj_id self, dictionary params) 
    {
        dictionary pilotData = getPilotRow(getStringObjVar(self, "ace_name"));
    ...
    private void handleTauntNewAttacker(obj_id attacker, dictionary pilotData) {
        if(pilotWillTaunt()) {
    ...
    private void handleTauntKnownAttacker(obj_id attacker, dictionary pilotData) {
        if(pilotWillTaunt()) {
    ...
    private void handleTauntVanquished(obj_id attacker, dictionary pilotData) {
        String taunt = VANQUISH_TAUNT + rand(1, TAUNT_QUANTITY);
        doTaunt(attacker, pilotData, taunt);
    }

    private void handleTauntDespawn(obj_id attacker, dictionary pilotData) {
        String taunt = DESPAWN_TAUNT + rand(1, TAUNT_QUANTITY);
        doTaunt(attacker, pilotData, taunt);
    }

    private void doTaunt(obj_id attacker, dictionary pilotData, String taunt) {
        obj_id self = getSelf();
    ...
    public int handleDespawn(obj_id self, dictionary pilotData) {
        String taunt = DESPAWN_TAUNT + rand(1, TAUNT_QUANTITY);
