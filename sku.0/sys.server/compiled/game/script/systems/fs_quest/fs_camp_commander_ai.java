...
public class fs_camp_commander_ai extends script.base_script
{
    ...
    public int msgSpawnRescuers(obj_id self, dictionary params)
    {
    ...
    public int msgDeathSequence(obj_id self, dictionary params)
    {
        if (!utils.hasScriptVar(self, "deathSequence"))
        {
            destroyObject(self);
    ...
    public int msgLifeExpired(obj_id self, dictionary params)
    {
        lifeExpired();
        return SCRIPT_CONTINUE;
    }
    ...
    public int msgPrimaryEscortDied(obj_id self, dictionary params)
    {
    ...
    public int msgShieldKillerOuster(obj_id self, dictionary params)
    {
        setObjVar(self, "fs_cs.shieldKillerOusted", 1);
        return SCRIPT_CONTINUE;
    }
    public int msgPrimaryEscortAbandoned(obj_id self, dictionary params)
    {
        ai_lib.aiStopFollowing(self);
        obj_id shieldKiller = getObjIdObjVar(self, fs_counterstrike.OBJVAR_CAMP_SHIELD_KILLER);
        if (!isIdValid(shieldKiller))
    ...
    public int msgRanAwaySuccessfully(obj_id self, dictionary params)
    {
        if (!hasObjVar(self, "fs_cs.iAmFree"))
        {
            return SCRIPT_CONTINUE;
    ...
    public int msgRun(obj_id self, dictionary params)
    {
        if (!hasObjVar(self, "fs_cs.iAmFree"))
        {
            return SCRIPT_CONTINUE;
