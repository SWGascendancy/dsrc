...
public class fs_quest_player extends script.base_script
{
    ...
    public int msgValidateFSQuestPhase(obj_id self, dictionary params)
    {
    ...
    public int msgSetQuestPhaseId(obj_id self, dictionary params)
    {
        boolean rslt = false;
        String key = "";
    ...
    public void resetQuest(obj_id self, int phase)
    {
        for (int i = 1; i <= 8; i++)
        {
    ...
    public boolean hasSurveyQuest(obj_id player)
    {
        boolean yesNo = false;
        for (int x = 1; x < 9; x++)
        {
    ...
    public void checkFsQuestCraftingBug(int phase)
    {
        obj_id self = getSelf();
        if (phase == 2)
        {
