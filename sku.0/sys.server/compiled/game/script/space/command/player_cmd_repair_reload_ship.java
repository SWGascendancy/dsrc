...
public class player_cmd_repair_reload_ship extends script.base_script
{
    ...
    public int OnSpaceUnitDocked(obj_id self, obj_id dockTarget)
    {
    ...
    public int OnSpaceUnitUnDocked(obj_id self, obj_id dockTarget, boolean dockSuccessful)
    {
        obj_id objPilot = getPilotId(dockTarget);
        if (!isIdValid(objPilot))
        {
    ...
    public int OnSpaceUnitStartUnDock(obj_id self, obj_id dockTarget)
    {
        obj_id objPilot = getPilotId(dockTarget);
        if (!isIdValid(objPilot))
        {
    ...
    public int OnSpaceUnitMoveToComplete(obj_id self)
    {
        obj_id commander = getObjIdObjVar(self, "cmd.repair.commander");
    ...
    public int repairReplyTimeout(obj_id self, dictionary params)
    {
        obj_id commander = getObjIdObjVar(self, "cmd.repair.commander");
        if (!isIdValid(commander))
        {
    ...
    public int dockTargetMovementCheck(obj_id self, dictionary params)
    {
        if (utils.hasLocalVar(self, "cmd.repair.buggingOut"))
        {
            return SCRIPT_CONTINUE;
    ...
    public int repairSFX(obj_id self, dictionary params)
    {
        obj_id pilot = params.getObjId("pilot");
        obj_id ship = params.getObjId("ship");
        if (!isIdValid(pilot) || !isIdValid(ship))
