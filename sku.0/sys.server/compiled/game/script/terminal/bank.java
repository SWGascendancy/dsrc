...
public class bank extends script.terminal.base.base_terminal
{
    ...
    public boolean openBankMenu(obj_id player)
    {
        obj_id self = getSelf();
        if (!isIdValid(player))
        {
    ...
    public int forceCloseBankSui(obj_id self, dictionary params)
    {
    ...
    public void closeBankTransferSui(obj_id self, obj_id player)
    {
        String scriptvar = SCRIPTVAR_BANK + "." + player;
        if (utils.hasScriptVar(self, scriptvar))
        {
