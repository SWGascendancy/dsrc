...
public class terminal_guild extends script.terminal.base.base_terminal
{
    ...
    public int getStructureGuildId(obj_id self)
    {
        int guildId = 0;
    ...
    public obj_id getMenuContextObjId(obj_id self, obj_id player, String varName)
    {
        deltadictionary dd = self.getScriptVars();
        return dd.getObjId("guildMenu." + player + "." + varName);
    }
    public void setMenuContextObjId(obj_id self, obj_id player, String varName, obj_id value)
    {
        deltadictionary dd = self.getScriptVars();
        dd.put("guildMenu." + player + "." + varName, value);
    }
    public int handleMakeLeader(obj_id self, dictionary params)
    {
    ...
    public int handleAcceptLeadership(obj_id self, dictionary params)
    {
        obj_id player = sui.getPlayerId(params);
    ...
    public void resetElection(obj_id self, obj_id player)
    {
        int guildId = getGuildId(player);
        if (guildId == 0)
        {
    ...
    public void startElection(obj_id self, obj_id player)
    {
        int guildId = getGuildId(player);
        if (guildId == 0)
        {
    ...
    public void registerToRun(obj_id self, obj_id player)
    {
        int guildId = getGuildId(player);
    ...
    public void unregisterFromRace(obj_id self, obj_id player)
    {
        int guildId = getGuildId(player);
        obj_id leader = guildGetLeader(guildId);
        if (!guild.isCandidate(guildId, player))
    ...
    public int handlerGuildScreenDestroyed(obj_id self, dictionary params)
    {
    ...
    public int showGuildsList(obj_id self, dictionary params)
    {
        if (params == null || params.isEmpty())
        {
            return SCRIPT_CONTINUE;
