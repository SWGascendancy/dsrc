...
public class lifeday_objective extends script.base_script
{
    ...
    private void treeDressed(obj_id self, obj_id player)
    {
    ...
    private void presentsStomped(obj_id self, obj_id player)
    {
        if (!factions.isDeclared(player))
        {
    ...
    private void giveTreat(obj_id player, int quantity, int faction)
    {
    ...
    private void applyCorrectBuff(obj_id self, obj_id player)
    {
    ...
    private boolean newDay(obj_id player)
    {
        if (hasObjVar(player, LIFEDAY_TIMESTAMP))
        {
            int now = getCalendarTime();
    ...
    private boolean canInteract(obj_id player, obj_id self)
    {
        int now = getCalendarTime();
        int secondsUntil = secondsUntilNextDailyTime(10, 0, 0);
        int then = now + secondsUntil;
    ...
    public int handleDressingCountdownTimer(obj_id self, dictionary params)
    {
    ...
    public int handleStompingCountdownTimer(obj_id self, dictionary params)
    {
        obj_id player = params.getObjId("player");
        if (!isIdValid(player))
        {
