...
public class lifeday_tree extends script.base_script
{
    ...
    private static final String LIFEDAY_BADGE = "lifeday_badge_08";
    private static final string_id TREE_BADGE = new string_id("spam", "tree_badge");
    private static final String LIFEDAY_YEAR = "gift.xmas05";

    public int OnObjectMenuRequest(obj_id self, obj_id player, menu_info mi)
    {
        if (!hasObjVar(player, LIFEDAY_YEAR) || isGod(player))
        {
    ...
    private boolean grantGift(obj_id player)
    {
        if (!isIdValid(player))
        {
            return false;
        ...
        setObjVar(player, LIFEDAY_YEAR, 1);
        sendSystemMessage(player, GIFT_GRANTED);
        CustomerServiceLog("grantGift", getFirstName(player) + "(" + player + ") has received his Christmas " + LIFEDAY_YEAR + " gift.");
        return true;
    }
}
