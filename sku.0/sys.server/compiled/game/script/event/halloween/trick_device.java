...
public class trick_device extends script.base_script
{
    ...
    private boolean canUse(obj_id player, obj_id device)
    {
        if (!inHouse(device))
        {
    ...
    private void startDevice(obj_id player)
    {
    ...
    private void playTrick(obj_id self, obj_id player, obj_id target, int trick)
    {
        if (!ai_lib.isInCombat(player) || !ai_lib.isInCombat(target))
        {
            ...
            if (scariestChance == trick)
            {
                ...
                event_perk.giveTreat(player, true);
            ...
            else if (scareChance < 51)
            {
                ...
                event_perk.giveTreat(player, false);
            }
            else
            {
    ...
    private boolean inHouse(obj_id device)
    {
    ...
    private void stopFog(obj_id device)
    {
        stopClientEffectObjByLabel(device, "halloweenFog");
        if (hasObjVar(device, "fogOn"))
        {
