...
public class planet_event_handler extends script.base_script
{
    ...
    private void checkLifeDayData(obj_id self)
    {
    ...
    private boolean newDay(obj_id self)
    {
        if (hasObjVar(self, EVENT_TIMESTAMP))
        {
    ...
    private void newTimeStamp(obj_id self)
    {
        int now = getCalendarTime();
    ...
    public int setUpEventLeaderBoard(obj_id self, dictionary params)
    {
    ...
    public int resetEventDataAfterDelay(obj_id self, dictionary params)
    {
    ...
    public int lifeDayDailyAlarm(obj_id self, dictionary params)
    {
        String lifedayRunning = getConfigSetting("GameServer", "lifeday");
        if (lifedayRunning != null && !lifedayRunning.equals("false"))
        {
    ...
    public int lifeDayScoreBoardUpdate(obj_id self, dictionary params)
    {
        if (params == null || params.isEmpty())
        {
            return SCRIPT_CONTINUE;
    ...
    private boolean setupInitialEventScores(obj_id planet, String scoreObjVar, int modifier, boolean faction)
    {
    ...
    private boolean checkForHolidayEventConfigs(obj_id planet)
    {
        if (!isValidId(planet) || !exists(planet))
        {
            return false;
