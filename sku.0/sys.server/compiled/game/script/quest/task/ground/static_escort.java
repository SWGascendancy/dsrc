...
public class static_escort extends script.quest.task.ground.base_task
{
    ...
    public int messageStaticEscortCheckDistance(obj_id self, dictionary params)
    {
    ...
    public int messageEscortTaskCreatureDied(obj_id self, dictionary params)
    {
    ...
    public boolean isEscortNearby(int questCrc, int taskId, obj_id player, obj_id escortTarget)
    {
    ...
    public void failEscortDistanceCheck(obj_id player, int questCrc, int taskId, obj_id escortTarget)
    {
        String baseObjVar = groundquests.getBaseObjVar(player, taskType, questGetQuestName(questCrc), taskId);
        if (!hasObjVar(player, baseObjVar + objvarBeenWarnedDistance))
        {
