...
public class fs_old_man_exit extends script.base_script
{
    ...
    public int approach(obj_id self, dictionary params)
    {
        obj_id owner = null;
    ...
    public int OnTriggerVolumeEntered(obj_id self, String volumeName, obj_id target)
    {
    ...
    public int approachFail(obj_id self, dictionary params)
    {
        if (hasObjVar(self, "contact"))
        {
            return SCRIPT_CONTINUE;
    ...
    public int timeout(obj_id self, dictionary params)
    {
        obj_id owner = null;
        owner = getObjIdObjVar(self, "old_man_final.holder");
        if ((isIdValid(owner)) && (exists(owner)))
