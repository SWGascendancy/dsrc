...
public class greeter extends script.theme_park.newbie_tutorial.tutorial_base
{
    ...
    public int initiateConvo(obj_id self, dictionary params)
    {
    ...
    public int handleLookAtExplanation(obj_id self, dictionary params)
    {
        obj_id player = getPlayer(self);
    ...
    public int handleConvoExplain(obj_id self, dictionary params)
    {
        obj_id player = getPlayer(self);
        if (!isInRoom(player, "r2"))
        {
    ...
    public int OnEndNpcConversation(obj_id self, obj_id player)
    {
        removeObjVar(self, "newbie.inConvo");
        messageTo(self, "handleOfferSupplies", null, 2, false);
        return SCRIPT_CONTINUE;
    }
    public int handleOfferSupplies(obj_id self, dictionary params)
    {
        obj_id player = getPlayer(self);
        if (!isInRoom(player, "r2"))
        {
