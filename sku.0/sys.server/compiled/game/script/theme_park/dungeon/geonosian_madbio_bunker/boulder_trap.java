...
public class boulder_trap extends script.base_script
{
    ...
    public boolean hasPlayer(int numContents, obj_id[] roomContents)
    {
        int x = 0;
    ...
    public int playedAnim(obj_id self, dictionary params)
    {
        setObjVar(getSelf(), "played", 1);
        messageTo(getSelf(), "resetAnim", null, 120, false);
        return SCRIPT_CONTINUE;
    }
    public int resetAnim(obj_id self, dictionary params)
    {
        if (hasObjVar(getSelf(), "played"))
        {
