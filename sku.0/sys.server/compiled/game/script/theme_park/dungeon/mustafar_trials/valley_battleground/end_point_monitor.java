...
public class end_point_monitor extends script.base_script
{
    ...
    public int scan(obj_id self, dictionary params)
    {
        checkForDroidArmy(self);
        return SCRIPT_CONTINUE;
    }
    public int getStage(obj_id self)
    {
        int stage = 0;
    ...
    public void incrementStage(obj_id self)
    {
        int stage = getStage(self);
        stage += 1;
        utils.setScriptVar(self, STAGE, stage);
    }
    public void decrementStage(obj_id self)
    {
        int stage = getStage(self);
        stage -= 1;
        if (stage < 0)
    ...
    public void checkForDroidArmy(obj_id self)
    {
