...
public class helper_droid extends script.base_script
{
    ...
    public int handleDroidInitiateGreeting(obj_id self, dictionary params)
    {
        obj_id master = getMaster(self);
        if (!isIdValid(master))
        {
    ...
    public void showProfessionsListbox(obj_id player, obj_id droid, string_id greetingEnd)
    {
        if (utils.hasScriptVar(player, "new_player.openSui"))
        {
            int oldSui = utils.getIntScriptVar(player, "new_player.openSui");
