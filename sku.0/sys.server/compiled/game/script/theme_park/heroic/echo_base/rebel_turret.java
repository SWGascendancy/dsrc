...
public class rebel_turret extends script.base_script
{
    ...
    public int InstanceFaction(obj_id self, dictionary params)
    {
        return SCRIPT_CONTINUE;
    }
    public int boredHothTurrets(obj_id self, dictionary params)
    {
        if (hasObjVar(self, PHASE_3_TURRET))
        {
            return SCRIPT_CONTINUE;
    ...
    public int delayedHothTurretShot(obj_id self, dictionary params)
    {
    ...
    public void hothTurretShotApplyDamage(obj_id turret, obj_id target, obj_id player)
    {
        if (!isIdValid(turret))
        {
            return;
    ...
    public float getRandomTriggerTime(float minTime, float maxTime)
    {
