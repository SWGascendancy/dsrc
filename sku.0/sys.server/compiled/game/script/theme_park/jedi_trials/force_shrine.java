...
public class force_shrine extends script.base_script
{
    ...
    public static final string_id LEVELUP_MENU = new string_id("jedi_trials", "level_up");
    public static final string_id ALREADY_MAX_LEVEL = new string_id("jedi_trials", "max_level");
    public static final string_id ALREADY_USED = new string_id("jedi_trials", "already_used");
    ...
    public int OnObjectMenuRequest(obj_id self, obj_id player, menu_info mi)
    {
        ...
        mi.addRootMenu(menu_info_types.SERVER_MENU8, LEVELUP_MENU);
    ...
    public int OnObjectMenuSelect(obj_id self, obj_id player, int item)
    {
        ...
        if (item == menu_info_types.SERVER_MENU8)
        {
            ...
            {
                sendSystemMessage(player, ALREADY_MAX_LEVEL);
            }
            else if (hasObjVar(tatooine, "free_90_" + getPlayerStationId(player)) && hasObjVar(tatooine, "free_90_2_" + getPlayerStationId(player)) && !isGod(player))
            {
                sendSystemMessage(player, ALREADY_USED);
            }
            else
            {
                if (!hasObjVar(tatooine, "free_90_" + getPlayerStationId(player)))	
                {
                    setObjVar(tatooine, "free_90_" + getPlayerStationId(player), true);
                    respec.autoLevelPlayer(player, 90, true);
                }
                else if (!hasObjVar(tatooine, "free_90_2_" + getPlayerStationId(player)))	
                {
                    setObjVar(tatooine, "free_90_2_" + getPlayerStationId(player), true);
                    respec.autoLevelPlayer(player, 90, true);
                }
            }
        }
    ...
    public int handleEnterTrialsChoice(obj_id self, dictionary params)
    {
    ...
    public boolean canMeditateHere(obj_id forceShrine, obj_id player)
    {
    ...
    public boolean isOnJediTrials(obj_id player)
    {
    ...
    public boolean hasNotFoundShrine(obj_id player)
    {
    ...
    public boolean isTargetShrine(obj_id self, obj_id player)
    {
    ...
    public boolean isTargetShrinePlanet(obj_id self, obj_id player)
    {
        location currentShrineLoc = getLocation(self);
        location targetShrineLoc = getLocationObjVar(player, jedi_trials.JEDI_TRIALS_SHRINELOC_OBJVAR);
        if (currentShrineLoc != null && targetShrineLoc != null)
    ...
    public void issuePadawanRobe(obj_id player)
    {
    ...
    public boolean canGetUltraCloak(obj_id player)
    {
        return (badge.hasBadge(player, "bdg_col_jedi_robe"));
    }
    public void issueUltraCloak(obj_id player, int robeType)
    {
        obj_id pInv = utils.getInventoryContainer(player);
        String robeName = "";
        String clientEffect = "";
