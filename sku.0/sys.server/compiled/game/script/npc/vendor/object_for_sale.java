...
public class object_for_sale extends script.base_script
{
    ...
    public boolean confirmLimit(obj_id self, obj_id player)
    {
    ...
    public boolean confirmFunds(obj_id self, obj_id player)
    {
        ...
        for (int i = 0; i < tokenCostInThisFunction.length; i++)
        {
            owedTokenCount += tokenCostInThisFunction[i];
        }
        if (owedTokenCount > 0)
        {
            owesTokens = true;
    ...
    public void processItemPurchase(obj_id self, obj_id player)
    {
    ...
    public string_id parseNameToStringId(String itemName, obj_id item)
    {
