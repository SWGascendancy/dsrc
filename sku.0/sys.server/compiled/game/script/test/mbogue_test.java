...
public class mbogue_test extends script.base_script
{
    ...
    public vector getRandomPosition(float radius)
    {
    ...
    public obj_id createUnit(obj_id self, String unitName, float radius, vector position)
    {
    ...
    public obj_id createUnit(obj_id self, String unitName, float radius)
    {
        obj_id unitId = createUnit(self, unitName, radius, vector.zero);
        return unitId;
    }
    public void debugSpaceUnitFollow(obj_id self, obj_id unit, obj_id followedUnit, vector direction, float offset)
    {
    ...
    public void debugDestroyObject(obj_id self, obj_id object)
    {
        destroyObject(object);
    }
    public void debugSpaceUnitAddPatrolPath(obj_id self, obj_id unit, transform[] path)
    {
        debugConsoleMsg(self, "spaceUnitAddPatrolPath() unit: " + unit + " path.length: " + path.length);
        ship_ai.spacePatrol(unit, path);
    }
    public transform[] createPatrolPathSpiral(vector position_w, float radius)
    {
        final int points = 60;
    ...
    public int createPatrollingSquad(obj_id self, String unitName, int squadSize, vector position_w)
    {
        final float createRadius = 200.0f;
