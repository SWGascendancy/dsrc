...
public class qa_pilot_roadmap_tatooine_imperial extends script.base_script
{
    ...
    public int OnQuestActivated(obj_id self, int questId)
    {
    ...
    public int handleImperialPilotMainMenuOptions(obj_id self, dictionary params)
    {
        if (isGod(self))
        {
    ...
    public boolean stepThroughPilot(obj_id self, int step)
    {
        if (step >= 0)
        {
    ...
    public void detachAndcleanAllScriptVars(obj_id self)
    {
        utils.removeScriptVarTree(self, SCRIPTVAR);
        detachScript(self, "test.qa_pilot_roadmap_tatooine_imperial");
    }
