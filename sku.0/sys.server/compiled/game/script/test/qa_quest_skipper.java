...
public class qa_quest_skipper extends script.base_script
{
    ...
    public int handleQuestOptions(obj_id self, dictionary params)
    {
        if (isGod(self))
        {
            if (utils.hasScriptVar(self, "qaquest.pid"))
    ...
    public int handleAddGroundQuestManually(obj_id self, dictionary params)
    {
        if (isGod(self))
        {
            if (utils.hasScriptVar(self, "qaquest.pid"))
    ...
    public int handleAddSpaceQuestManually(obj_id self, dictionary params)
    {
        if (isGod(self))
        {
            if (utils.hasScriptVar(self, "qaquest.pid"))
    ...
    public int revokeQuestHandler(obj_id self, dictionary params)
    {
    ...
    public int grantQuestHandler(obj_id self, dictionary params)
    {
    ...
    public int grantAndCompleteQuestHandler(obj_id self, dictionary params)
    {
        String widgetName = params.getString("eventWidgetName");
        String textData = params.getString("Prompt.lblPrompt.LocalText");
        if (widgetName.equalsIgnoreCase("%button0%"))
    ...
    public int delay(obj_id self, dictionary params)
    {
        return SCRIPT_CONTINUE;
    }
    public void showToolMainMenu(obj_id self)
    {
        try
        {
    ...
    public void cleanAllScriptVars(obj_id self)
    {
        utils.removeScriptVarTree(self, "qaquest");
    }
    public boolean getTestQuests(obj_id self)
    {
    ...
    public String getQuestString(obj_id self)
    {
    ...
    public String[] getCorrectMenu(obj_id self, String questStringAndName)
    {
        if (!questStringAndName.equals(""))
        {
    ...
    public void bulkGrantAndCompleteQuestUi(obj_id self, String textData)
    {
    ...
    public void bulkGrantQuestUi(obj_id self, String textData)
    {
        String uiTitle = "Quest Macro";
        int page = createSUIPage("/Script.messageBox", self, self);
        if (!textData.equals(""))
    ...
    public void iterateGrantQuestStrings(obj_id self, String[] allData, boolean completeFlag)
    {
        if (allData.length > 0)
        {
            for (int i = 0; i < allData.length; i++)
    ...
    public void iterateClearQuestStrings(obj_id self, String[] allData)
    {
        if (allData.length > 0)
        {
            for (int i = 0; i < allData.length; i++)
