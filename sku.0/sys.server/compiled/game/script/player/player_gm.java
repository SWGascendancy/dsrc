...
public class player_gm extends script.base_script
{
    ...
    public int cmdGenerateCraftedItem(obj_id self, obj_id target, String params, float defaultTime)
    {
        if (!isGod(self))
        {
            return SCRIPT_CONTINUE;
        ...
        if (st.countTokens() != 3)
        {
            ...
            sendSystemMessageTestingOnly(self, "Correct usage is /generateCraftedItem <schematic name> <quality-percentage(integer)> <count(integer)>");
        ...
        String item_count = st.nextToken().toLowerCase();
        int attributePercentage = utils.stringToInt(percentage_string);
        int item_total_count = utils.stringToInt(item_count);
        ...
        if (inventory != null)
        {
            ...
            setCrafter(item, creationTarget);
            setCount(item, item_total_count);
