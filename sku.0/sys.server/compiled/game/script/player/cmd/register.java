...
public class register extends script.base_script
{
    ...
    public int cmdRegisterWithLocation(obj_id self, obj_id target, String params, float defaultTime)
    {
        ...
        if (!isIdValid(topMost) || topMost == self)
        {
            ...
            if (isIdValid(camp))
            {
                if (hasSkill(self, "social_dancer_novice") || hasSkill(self, "social_musician_novice") || hasSkill(self, "science_doctor_novice") || hasSkill(self, "class_entertainer_phase1_novice"))
                {
                    updateTarget = camp;
                }
            }
