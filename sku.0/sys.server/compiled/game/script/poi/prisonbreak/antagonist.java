...
public class antagonist extends script.poi.base.scenario_actor
{
    ...
    public void minionChatter(obj_id self, String convo, obj_id speaker, obj_id poiMaster)
    {
        int idx = rand(1, 4);
    ...
    public int attackWallLeader(obj_id self, dictionary params)
    {
        poi.quickSay(self, chat.CHAT_EXCLAIM, chat.MOOD_DEVIOUS, "a_attackwall_leader");
        return SCRIPT_CONTINUE;
    }
    public int attackEveryone(obj_id self, dictionary params)
    {
        obj_id poiMaster = poi.getBaseObject(self);
        if ((poiMaster == null) || (poiMaster == obj_id.NULL_ID))
        {
