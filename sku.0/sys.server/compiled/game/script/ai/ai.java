...
public class ai extends script.base_script
{
    ...
    public void doDefaultCalmBehavior(obj_id npc)
    {
    ...
    public void goAlert(obj_id npc)
    {
        if (ai_lib.isAiDead(npc))
        {
            return;
    ...
    public void goThreaten(obj_id npc)
    {
        if (ai_lib.isAiDead(npc))
        {
            return;
    ...
    public boolean defendLair(obj_id npc, obj_id defender, obj_id[] attackers, boolean isInCombat)
    {
        if (isMob(defender))
        {
            return false;
    ...
    public int aiCorpsePrepared(obj_id self, dictionary params)
    {
        ...
        {
            if (isIdValid(pks[i]) && (corpseLevel + 5 < getLevel(pks[i])))
            {
                doNotDropCard = true;
            }
        ...
        for (int i = 0, j = pks.length; i < j; i++)
        {
            if (isIdValid(pks[i]))
            {
                ...
                {
                    sendSystemMessageTestingOnly(pks[i], "QA TCG COMBAT.  Do not drop card? " + doNotDropCard + " hasCardDelay? " + scheduled_drop.hasCardDelay(pks[i], sourceSystem) + " bad level? " + (corpseLevel + 5 < getLevel(pks[i])));
                }
            }
        }
        if (!doNotDropCard)
    ...
    public int removeMeatlumpRecruitmentScriptVar(obj_id self, dictionary params)
    {
        utils.removeScriptVar(self, "collection.recruited");
        return SCRIPT_CONTINUE;
    }
    public int removeMeatlumpInvestigatorScriptVar(obj_id self, dictionary params)
    {
