...
public class familiar extends script.base_script
{
    ...
    public void doFamiliarTrick(obj_id pet, int trickNum)
    {
        ...
        else if (creatureName.equals("familiar_gungan_male_01"))
        {
            trickNum = trickNum + 4;
        }
    ...
    public int doFamiliarTrick(obj_id self, dictionary params)
    {
    ...
    public boolean applyBuff(obj_id pet)
    {
        obj_id master = getMaster(pet);
        if (!isIdValid(master))
        {
    ...
    public int cleanupCheck(obj_id self, dictionary params)
    {
        if (!pet_lib.findMaster(self))
        {
            destroyObject(self);
