...
public class planetary_base_register extends script.base_script
{
    ...
    public int performQuery(obj_id self, dictionary params)
    {
        queryPlanetaryBaseData(self);
        return SCRIPT_CONTINUE;
    }
    private void setControlData(obj_id self)
    {
        utils.setScriptVar(getPlanetByName(getLocation(self).area), gcw.GCW_BASE_MANAGER, self);
    }
    public int OnClusterWideDataResponse(obj_id self, String manage_name, String dungeon_name, int request_id, String[] element_name_list, dictionary[] dungeon_datas, int lock_key)
    {
    ...
    private void queryPlanetaryBaseData(obj_id self)
    {
        getClusterWideData("gcw_player_base", "base_cwdata_manager*", true, self);
    }
    private void addToBaseCount(obj_id self, int rebel, int imperial)
    {
    ...
    public int alterBaseCount(obj_id self, dictionary params)
    {
