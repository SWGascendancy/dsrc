...
public class objective_power_regulator extends script.faction_perk.hq.objective_object
{
    ...
    private void setForOverload(obj_id self, obj_id player)
    {
    ...
    public int handleOverloadSui(obj_id self, dictionary params)
    {
    ...
    private void randomizeSwitches(obj_id self)
    {
    ...
    private boolean[] flipSwitch(boolean[] states, int[] rules, int idx)
    {
        if (states == null || states.length == 0)
        {
            return null;
