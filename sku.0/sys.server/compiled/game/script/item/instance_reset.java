...
public class instance_reset extends script.base_script
{
    ...
    private boolean deviceIsUsable(obj_id self, obj_id player) {
    ...
    public int onInstanceResetItemResponse(obj_id self, dictionary params)
    {
    ...
    public int handlerConfirmResetInstance(obj_id self, dictionary params)
    {
        if (params == null || params.isEmpty())
        {
            return SCRIPT_CONTINUE;
        ...
        if (getTemplateName(self).equals("object/tangible/veteran_reward/data_terminal_s3.iff"))
            setObjVar(player, "neural_cd", getCalendarTime() + 86400);
        else
            destroyObject(self);
        return SCRIPT_CONTINUE;
    }
