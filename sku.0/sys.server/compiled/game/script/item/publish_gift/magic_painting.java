...
public class magic_painting extends script.base_script
{
    ...
    public int createPainting(obj_id self, dictionary params)
    {
        obj_id house = getTopMostContainer(self);
    ...
    public int switchPainting(obj_id self, dictionary params)
    {
        if (hasObjVar(self, "paintingId"))
        {
    ...
    public int intializePaintingController(obj_id self, dictionary params)
    {
        if (!utils.isNestedWithinAPlayer(self))
        {
            obj_id house = getTopMostContainer(self);
