...
public class serving_droid_terminal extends script.base_script
{
    ...
    public int destroyDroid(obj_id self, dictionary params)
    {
        obj_id terminal = self;
        if (!isValidId(terminal))
        {
    ...
    public boolean nameTerminal(obj_id terminal)
    {
        if (!isValidId(terminal))
        {
            return false;
    ...
    public boolean setDroidVars(obj_id terminal, obj_id droid)
    {
    ...
    public boolean createDroid(obj_id terminal, obj_id player)
    {
        if (!isValidId(terminal) || !exists(terminal))
        {
            return false;
    ...
    public boolean recreateDroid(obj_id terminal)
    {
        obj_id anotherDroid = getObjIdObjVar(terminal, DROID_ID);
        if (isValidId(anotherDroid) && exists(anotherDroid))
        {
