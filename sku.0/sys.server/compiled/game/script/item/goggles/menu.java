...
public class menu extends script.base_script
{
    ...
    public int createNewGoggles(obj_id self, obj_id target, String params, float defaultTime)
        {
        if (getCurrentBirthDate() - getPlayerBirthDate(self) < 90 && !isGod(self))
        {
            sendSystemMessageTestingOnly(self, "Your character must be at least 90 days old in order to get the Special Edition Goggles.");
            return SCRIPT_CONTINUE;
        }
        obj_id[] items = new obj_id[COLLECTOR_EDITION_ITEMS.length];
        for (int i = 0; i < items.length; i++)
            items[i] = createObjectInInventoryAllowOverload(COLLECTOR_EDITION_ITEMS[i], self);
    ...
    public int handleColorizeFrame(obj_id self, dictionary params)
    {
        utils.removeScriptVar(self, SCRIPTVAR_FRAME_PID);
    ...
    public int handleColorizeLens(obj_id self, dictionary params)
    {
        utils.removeScriptVar(self, SCRIPTVAR_LENS_PID);
        int bp = sui.getIntButtonPressed(params);
        if (bp == sui.BP_CANCEL)
    ...
    public int validateGoggles(obj_id self, dictionary params)
    {
    ...
    public boolean isSEGoggle(obj_id item)
    {
